...
public class firstRun extends Driver{
	...
	public void first_Run() throws InterruptedException
	{
		...
		try
		{
			...
			ATUReports.add("Alert present and handled", true);

		}   // try
		catch (NoAlertPresentException Ex)
		{
			System.out.println("Alert are not appear");
			ATUReports.add("Alert not present handled through Try catch ", true);

		}   catch (NoSuchElementException Ex)
		{  
			System.out.println("Element is not present");
			ATUReports.add("Element is not present handled through Try catch ", true);
		}  
...
//			    FMF.
		...
		try{
			MobileElement Address = (MobileElement) Ad.findElementByXPath("//UIAApplication[1]/UIAWindow[1]/UIANavigationBar[1]/UIACollectionView[1]/UIACollectionCell[1]/UIAStaticText[1]");
